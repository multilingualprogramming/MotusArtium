...
function setActiveTier(tier, options = {}) {
        ...
        const _polyglot = () => window.ui?.visualisations?.polyglot_studio_vue || null;

        function renderPolyglotSurface(selectedEntity, labels, lang, entityId) {
            return _polyglot()?.rendre_surface_polyglotte?.(selectedEntity, labels, lang, entityId) || "";
        }
        ...
        const _chron = () => window.ui?.visualisations?.chronologie_vue || null;

        function fieldDate(record, keys) {
            const c = _chron();
            return c ? c.champ_date(record, keys) : "";
        }

        function extractYear(dateValue) {
            const c = _chron();
            return c ? c.extraire_annee(dateValue) : null;
        }

        function formatTimelineYear(year) {
            const c = _chron();
            return c ? c.formater_annee(year) : "";
        }
